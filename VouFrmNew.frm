   ...
   Begin VB.Frame Frame6 
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.ComboBox ComboVouType 
            ...
            ForeColor       =   &H00FF0000&
            Height          =   360
            ItemData        =   "VouFrmNew.frx":0000
            Left            =   915
            List            =   "VouFrmNew.frx":000D
...
Dim Lcashbankacccount As Integer:   Dim VoutIDStr As String
...
'        Exit Sub
    ...
    If LenB(DataCombo1.BoundText) > 0 Then
        ...
        If (txthiddenid.text = "0" Or txthiddenid.text = "") Then
            ...
            VoutIDStr = VoutIDStr + ";" + CStr(RowVouId)
        Else
            ...
            While Not TempVch.EOF
                If (TempVch!VOU_ID = txthiddenid.text) Then
                    ...
                    End If
                    TempVch.Update
                    VoutIDStr = VoutIDStr + ";" + txthiddenid.text
                End If
                TempVch.MoveNext
            Wend
        End If
...
Sub Delete_Entry()
    ...
    'sACHIN -- to manage voucher log
    If VoutIDStr = "" Then
        mysql = "INSERT INTO VCHAMT_LOG (CompCode , VOU_NO, VOU_TYPE, VOU_DT, DR_CR, AC_CODE,AMOUNT , CHEQUE_NO, CHEQUE_DT, NARRATION, BRANCH, BANK_NAME, vouid, invno, EXCODE, IAMOUNT, CURRRATE, EXID, VOU_ID, ACCID,loginuser,datetm,[tran])  "
        mysql = mysql & "SELECT CompCode , VOU_NO, VOU_TYPE, VOU_DT, DR_CR, AC_CODE,AMOUNT , CHEQUE_NO, CHEQUE_DT, NARRATION, BRANCH, BANK_NAME, vouid, invno, EXCODE, IAMOUNT, CURRRATE, EXID, VOU_ID, ACCID,'" & GUserName & "',getdate(),'3' FROM VCHAMT WHERE COMPCODE = " & GCompCode & " AND VOU_NO='" & F_VOU_NO_OLD & "' "
        Cnn.Execute mysql
    Else
        VoutIDStr = Trim(VoutIDStr)
        VoutIDStr = Mid(VoutIDStr, 2, Len(VoutIDStr))
        Dim LLists() As String
        Dim LCount As Integer
        Dim I As Integer
        LLists = Split(VoutIDStr, ";")
        LCount = UBound(LLists)
        For I = 0 To LCount
            If LLists(I) <> "" Then
                mysql = "INSERT INTO VCHAMT_LOG (CompCode , VOU_NO, VOU_TYPE, VOU_DT, DR_CR, AC_CODE,AMOUNT , CHEQUE_NO, CHEQUE_DT, NARRATION, BRANCH, BANK_NAME, vouid, invno, EXCODE, IAMOUNT, CURRRATE, EXID, VOU_ID, ACCID,loginuser,datetm,[tran])  "
                mysql = mysql & "SELECT CompCode , VOU_NO, VOU_TYPE, VOU_DT, DR_CR, AC_CODE,AMOUNT , CHEQUE_NO, CHEQUE_DT, NARRATION, BRANCH, BANK_NAME, vouid, invno, EXCODE, IAMOUNT, CURRRATE, EXID, VOU_ID, ACCID,'" & GUserName & "',getdate(),'2' FROM VCHAMT WHERE COMPCODE = " & GCompCode & " AND VOU_NO='" & F_VOU_NO_OLD & "' AND INVNO='" & LLists(I) & "' "
                Cnn.Execute mysql
            End If
        Next I
    End If
